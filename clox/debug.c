...
static int constantInstruction(const char *name, Chunk *chunk, int offset) {
  // The next item in the chunk's code array is the index of the constant in the
...
static int invokeInstruction(const char *name, Chunk *chunk, int offset) {
  uint8_t constant = chunk->code[offset + 1]; // Name of the method
...
int disassembleInstruction(Chunk *chunk, int offset) {
  ...
  switch (instruction) {
    ...
    case OP_CLOSURE: {
      ...
      // The first operand is the function itself
    ...
    case OP_EQUAL_PRESERVE:
      return simpleInstruction("OP_EQUAL_PRESERVE", offset);
    case OP_GREATER:
